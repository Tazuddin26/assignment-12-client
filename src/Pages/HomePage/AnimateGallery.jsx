...
export default function AnimateGallery() {
  ...
  return (
    <div className="relative flex flex-col justify-end h-[400px] bg-gray-100 lg:p-10 p-4">
      <motion.div
        className="absolute top-0 left-0 w-full h-80 flex items-center justify-center font-tauri text-amber-500 text-3xl font-bold"
      ...
      <div className="lg:z-10 lg:w-8/12 w-full lg:ml-56 justify-center grid grid-cols-3 md:grid-cols-3 lg:grid-cols-3 lg:gap-6 gap-2 ">
        {features.map((feature, index) => (
          <motion.div
            key={feature.id}
            className="lg:p-10 p-2 bg-white font-abel text-center shadow-lg rounded-lg cursor-pointer hover:bg-green-500 hover:text-white transition-all duration-300"
            whileHover={{ scale: 1.05 }}
            onMouseEnter={() => setActiveFeature(index)}
            onMouseLeave={() => setActiveFeature(0)}
          >
            <h3 className="lg:text-xl font-semibold items-center">
              {feature.title}
            </h3>
